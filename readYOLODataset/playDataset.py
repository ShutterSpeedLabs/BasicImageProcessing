...
class YoloViewer(QWidget):
    def __init__(self):
        ...
        self.play_button = QPushButton('Play All', self)
        self.play_button.clicked.connect(self.play_all_images)
    ...
    def play_all_images(self):
        if not self.image_files:
            return

        # Start the timer to play images
    ...
    def timer_event(self):
        ...
        if self.current_index >= len(self.image_files) - 1:
            self.timer.stop()
            self.play_button.setEnabled(True)  # Enable play button after playing
            return
